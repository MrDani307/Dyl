...
function obfuscateCode(sourceCode) {
    const tempIn = path.join(__dirname, `temp_in_${Date.now()}.lua`);
    const tempOut = path.join(__dirname, `temp_out_${Date.now()}.lua`);
    
    try {
        fs.writeFileSync(tempIn, sourceCode, 'utf8');
        
        execSync(`lua ./obfuscator/cli.lua "${tempIn}" --output "${tempOut}"`, { stdio: 'ignore' });
        
        if (fs.existsSync(tempOut)) {
            const result = fs.readFileSync(tempOut, 'utf8');
            pcallDelete(tempIn);
            pcallDelete(tempOut);
            return result;
        }
    } catch (e) {
        console.error("Obfuscation failed, sending raw with wrapper:", e);
    }
...
app.post('/get-feature', (req, res) => {
    const { feature, userId, jobId } = req.body;
    ...
    else if (feature === "palette_builder") {
        rawCode = `
    ...
    else if (feature === "anims_and_loops") {
        rawCode = `
    ...
    else if (feature === "color_structure") {
        rawCode = `
    ...
    else if (feature === "create_prediction") {
        rawCode = `
    ...
    const boundCode = `
        if game.Players.LocalPlayer.UserId ~= ${userId} then return end
        if game.JobId ~= "${jobId}" then return end
        ${rawCode}
    `;

    const finalObfuscatedCode = obfuscateCode(boundCode);

    res.json({ code: finalObfuscatedCode });
});
